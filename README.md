# 📊 Data Analytics Portfolio  
### By: **AMINULLAHI, Abdulkabir Adebayo**

Welcome to my Data Analytics Portfolio!  
Here, I showcase projects where I use **Google BigQuery**, **BigQuery Standard SQL**, **Tableau**, and **data storytelling** to drive insights and solve analytical problems.

---

# 🔍 Featured Projects

## **1. Bellabeat Wellness Case Study**  
**Tools:** BigQuery • SQL • Tableau • CSV Data  
**Skills:** Data cleaning, UNIONs, JOINs, aggregation, visualization, storytelling  

This project explores user activity, sleep, and calorie-burn behaviors using Fitbit data to help Bellabeat improve wellness-focused marketing strategies.

### 🔗 Project Files  
- [Full Report (PDF)](Bellabeat_Report_AMINULLAHI_Abdulkabir_Adebayo.pdf)  
- [Slide Deck](Bellabeat_Slides_AMINULLAHI_Abdulkabir_Adebayo.pptx)  
- [Markdown Report](Bellabeat_Report_AMINULLAHI_Abdulkabir_Adebayo.md)  
- [Dashboard (PDF)](Bellabeat_Dashboard.pdf)

### 📸 Dashboard Visuals  
<p align="center">
  <img src="C:/Users/hp/Downloads/screenshorts/steps_trend.png" width="60%">
  <br><em>Daily Average Steps Over Time</em>
</p>

<p align="center">
  <img src="C:/Users/hp/Downloads/screenshorts/sleep_vs_steps.png" width="60%">
  <br><em>Sleep Duration vs Daily Step Count</em>
</p>

<p align="center">
  <img src="C:/Users/hp/Downloads/screenshorts/calories_trend.png" width="60%">
  <br><em>Daily Average Calories Burned Over Time</em>
</p>

<p align="center">
  <img src="C:/Users/hp/Downloads/screenshorts/user_segments.png" width="60%">
  <br><em>Distribution of User Activity Levels</em>
</p>

---

# 📁 Other Projects (Coming Soon)
More SQL, Tableau, and analytics projects will be added soon.

---

# 🧰 Tools & Technologies
- **Google BigQuery**  
- **BigQuery Standard SQL**  
- **Tableau**  
- **Google Sheets / Excel**  
- **CSV Files**  

---

# 👨🏽‍💻 About Me  
I am a data analyst passionate about transforming raw data into meaningful insights.  
I specialize in:

- SQL, BigQuery, Google sheets, Excel & R  
- Data Visualization using Tableau   
- Storytelling with data  
- Analytics project execution  

This portfolio highlights the journey of my technical skills, analysis style, and problem-solving approach.

---

# 📫 Contact  
Feel free to connect or reach out!  
- **Email:** *aminullahiabdulkabir170@gmail.com*  
- **LinkedIn:** **  
